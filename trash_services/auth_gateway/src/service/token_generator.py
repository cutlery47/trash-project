...
class TokenGenerator:
    ...
    def generate_access(self, id_: str, email: str, role: str) -> str:
        ...
        access_token = jwt.encode(payload=payload, algorithm=self.algorithm, key=SECRET_KEY)
        return access_token

    def generate_refresh(self, id_: str) -> str:
        payload = self._generate_refresh_payload(id_)
        refresh_token = jwt.encode(payload=payload, algorithm=self.algorithm, key=SECRET_KEY)
